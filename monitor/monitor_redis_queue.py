        ...
        if length is None:
            ...
            if length is not None:
                ...
                if length is not None:
                    ...
                    logging.warning(
                        f"Key '{key_to_check}', '{key_to_check_legacy}', or '{key_to_check_v4}' not found or not a list. Assuming length 0."
                    )
                    return 0
        return length
    except redis.exceptions.ResponseError as e:
        logging.error(
            f"Redis error when checking length of '{key_to_check}': {e}. Assuming length 0."
        )
        return 0
    except Exception as e:
        logging.error(
            f"Unexpected error when checking length of '{key_to_check}': {e}. Assuming length 0."
        )
        return 0
...
if __name__ == "__main__":
    ...
    if redis_conn:
        logging.info(
            f"Monitoring Redis queue '{QUEUE_NAME_PREFIX}:{QUEUE_NAME}' every {POLL_INTERVAL_SECONDS} seconds..."
        )
        ...
        try:
            while True:
                ...
                if length > 0:
                    logging.info(
                        f"Queue '{QUEUE_NAME_PREFIX}:{QUEUE_NAME}' length: {length}"
                    )
                elif (
                    last_known_length is not None
                    and last_known_length > 0
                    and length == 0
                ):
                    # Log when queue drains to zero (transition event)
                    logging.info(
                        f"Queue '{QUEUE_NAME_PREFIX}:{QUEUE_NAME}' drained to 0"
                    )
