...
N8N_WORKER_RUNNER_SERVICE_NAME = os.getenv(
    "N8N_WORKER_RUNNER_SERVICE_NAME", "n8n-worker-runner"
)  # n8n 2.0 task runner sidecar
...
def get_queue_length(r_conn):
    ...
    try:
        ...
        if length is not None:
            logging.debug(
                f"Using BullMQ v4+ key pattern '{key_to_check_v4}' for queue length."
            )
            return length
        ...
        if length is not None:
            logging.debug(
                f"Using legacy key pattern '{key_to_check_legacy}' for queue length."
            )
            return length

        logging.warning(
            f"Queue key patterns ('{key_to_check}', '{key_to_check_v4}', '{key_to_check_legacy}') not found or not a list. Assuming length 0."
        )
        return 0
    except redis.exceptions.ResponseError as e:
        logging.error(
            f"Redis error when checking length of queue keys: {e}. Assuming length 0."
        )
        return 0
    except Exception as e:
        logging.error(
            f"Unexpected error checking queue length: {e}. Assuming length 0."
        )
        return 0
...
def get_current_replicas(docker_client, service_name, project_name):
    ...
    if not project_name:
        logging.warning(
            "COMPOSE_PROJECT_NAME is not set. Cannot accurately determine current replicas."
        )
    ...
    try:
        ...
        service_containers = docker_client.containers.list(
            filters=filters, all=True
        )  # all=True to catch restarting ones too
        ...
        for container in service_containers:
            if container.status == "running":
                running_count += 1
        logging.debug(
            f"Found {running_count} running containers for service '{service_name}' in project '{project_name}'."
        )
        return running_count
    except Exception as e:
        logging.error(
            f"Error getting current replicas for {service_name} in {project_name}: {e}"
        )
        return None
...
def scale_service(service_name, replicas, compose_file, project_name):
    ...
    if not project_name:
        logging.error(
            "COMPOSE_PROJECT_NAME is not set. Cannot execute docker-compose scale."
        )
        return False
    ...
    try:
        ...
        logging.error(
            f"Timeout ({SUBPROCESS_TIMEOUT_SECONDS}s) scaling service {service_name} to {replicas}. Docker may be unresponsive."
        )
        return False
        ...
        logging.error(
            "docker-compose command not found. Ensure it's installed in the autoscaler container and in PATH."
        )
        return False
...
def scale_worker_with_runner(replicas, compose_file, project_name):
    ...
    command = [
        ...
        "--no-deps",
        "--scale",
        f"{N8N_WORKER_SERVICE_NAME}={replicas}",
        "--scale",
        f"{N8N_WORKER_RUNNER_SERVICE_NAME}={replicas}",
    ...
    try:
        ...
        logging.error(
            f"Timeout ({SUBPROCESS_TIMEOUT_SECONDS}s) scaling worker+runner to {replicas}. Docker may be unresponsive."
        )
        return False
    except subprocess.CalledProcessError as e:
...
def main():
    ...
    if not COMPOSE_PROJECT_NAME:
        logging.error(
            "CRITICAL: COMPOSE_PROJECT_NAME environment variable is not set. Autoscaler cannot function correctly."
        )
        logging.error(
            "Please set COMPOSE_PROJECT_NAME to the name of your Docker Compose project (usually the directory name)."
        )
    ...
    logging.info(
        f"Autoscaler started. Monitoring n8n worker service '{N8N_WORKER_SERVICE_NAME}' in project '{COMPOSE_PROJECT_NAME}'."
    )
    ...
    logging.info(
        f"  Polling Interval: {POLLING_INTERVAL_SECONDS}s, Cooldown: {COOLDOWN_PERIOD_SECONDS}s"
    )
    ...
    while True:
        try:
            ...
            if (current_time - last_scale_time) < COOLDOWN_PERIOD_SECONDS:
                logging.debug(
                    f"In cooldown period. Next check in {COOLDOWN_PERIOD_SECONDS - (current_time - last_scale_time):.0f}s."
                )
                time.sleep(POLLING_INTERVAL_SECONDS)  # Still sleep for polling interval
                continue

            queue_len = get_queue_length(r_conn)
            current_reps = get_current_replicas(
                docker_cl, N8N_WORKER_SERVICE_NAME, COMPOSE_PROJECT_NAME
            )

            if current_reps is None:
                logging.warning(
                    "Skipping scaling check because current replicas could not be determined."
                )
            ...
            if queue_len > 0:
                logging.info(
                    f"Queue Length: {queue_len}, Current Replicas: {current_reps}"
                )

            # Log replica changes (state-based tracking)
            if last_known_replicas is not None and current_reps != last_known_replicas:
                logging.info(
                    f"{N8N_WORKER_SERVICE_NAME} replicas changed: {last_known_replicas} -> {current_reps}"
                )
            last_known_replicas = current_reps

            if queue_len > SCALE_UP_QUEUE_THRESHOLD and current_reps < MAX_REPLICAS:
                new_replicas = min(
                    current_reps + 1, MAX_REPLICAS
                )  # Scale one by one for now
                logging.info(
                    f"Condition met for SCALE UP. Queue: {queue_len} > {SCALE_UP_QUEUE_THRESHOLD}. Replicas: {current_reps} < {MAX_REPLICAS}."
                )
                if scale_worker_with_runner(
                    new_replicas, COMPOSE_FILE_PATH, COMPOSE_PROJECT_NAME
                ):
                ...
                logging.info(
                    f"Condition met for SCALE DOWN. Queue: {queue_len} < {SCALE_DOWN_QUEUE_THRESHOLD}. Replicas: {current_reps} > {MIN_REPLICAS}."
                )
                if scale_worker_with_runner(
                    new_replicas, COMPOSE_FILE_PATH, COMPOSE_PROJECT_NAME
                ):
